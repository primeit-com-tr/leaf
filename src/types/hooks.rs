...
mod tests {
    ...
    fn render_hooks_returns_none_for_none() {
        ...
        let rendered = hooks.get_pre_prepare_deployment(&ctx).unwrap();
    ...
    fn render_hooks_renders_templates() {
        let hooks = Hooks {
            pre_prepare_deployment: Some(vec!["Hello {{ name }}".into()]),
            ..Default::default()
        };
    ...
    fn render_hooks_handles_multiple_scripts() {
        let hooks = Hooks {
            pre_prepare_deployment: Some(vec![
                "Hello {{ name }}".into(),
                "Goodbye {{ name }}".into(),
            ]),
            ..Default::default()
    ...
    fn render_hooks_fails_on_invalid_template() {
        let hooks = Hooks {
            pre_prepare_deployment: Some(vec!["{{ unclosed".into()]),
            ..Default::default()
        };

        let ctx = Context::new();

        let result = hooks.get_pre_prepare_deployment(&ctx);
